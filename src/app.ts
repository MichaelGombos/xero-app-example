...
} from "xero-node/dist/gen/model/assets/models";
import {
  Amount,
  ChargeType,
  ...
  private routes(): void {
    ...
    router.post("/change_organisation", async (req: Request, res: Response) => {
      try {
        ...
        const picked = xero.tenants.filter(
          (tenant) => tenant.tenantId == activeOrgId
        )[0];
    ...
    router.get("/disconnect", async (req: Request, res: Response) => {
      try {
        const updatedTokenSet: TokenSetParameters = await xero.disconnect(
          req.session.activeTenant.id
        );
        await xero.updateTenants(false);

        if (xero.tenants.length > 0) {
          const decodedIdToken: XeroIdToken = jwtDecode(
            updatedTokenSet.id_token
          );
          const decodedAccessToken: XeroAccessToken = jwtDecode(
            updatedTokenSet.access_token
          );
          req.session.decodedIdToken = decodedIdToken;
          req.session.decodedAccessToken = decodedAccessToken;
    ...
    router.post("/webhooks", async (req: Request, res: Response) => {
      ...
      this.verifyWebhookEventSignature(req)
        ? res.status(200).send()
        : res.status(401).send();
    });
    ...
    router.get("/accounts", async (req: Request, res: Response) => {
      try {
        ...
        const accountCreateResponse = await xero.accountingApi.createAccount(
          req.session.activeTenant.tenantId,
          account
        );
        ...
        // GET ONE
        const accountGetResponse = await xero.accountingApi.getAccount(
          req.session.activeTenant.tenantId,
          accountId
        );

        // UPDATE
        const accountUp: Account = {
          name: "Bar" + Helper.getRandomNumber(1000000),
        };
        const accounts: Accounts = { accounts: [accountUp] };
        const accountUpdateResponse = await xero.accountingApi.updateAccount(
          req.session.activeTenant.tenantId,
          accountId,
          accounts
        );
        ...
        const accountAttachmentsResponse: any =
          await xero.accountingApi.createAccountAttachmentByFileName(
            req.session.activeTenant.tenantId,
            accountId,
            filename,
            readStream,
        ...
        // GET ATTACHMENTS
        const accountAttachmentsGetResponse =
          await xero.accountingApi.getAccountAttachments(
            req.session.activeTenant.tenantId,
            accountId
          );

        // GET ATTACHMENT BY ID
        const accountAttachmentsGetByIdResponse =
          await xero.accountingApi.getAccountAttachmentById(
            req.session.activeTenant.tenantId,
            accountId,
            attachmentId,
            contentType
        ...
        fs.writeFile(
          `img-temp-${filename}`,
          accountAttachmentsGetByIdResponse.body,
          (err) => {
            if (err) {
              throw err;
            }
        ...
        const accountAttachmentsGetByFilenameResponse =
          await xero.accountingApi.getAccountAttachmentByFileName(
            req.session.activeTenant.tenantId,
            accountId,
            filename,
            contentType
        ...
        fs.writeFile(
          `img-temp-${filename}`,
          accountAttachmentsGetByFilenameResponse.body,
          (err) => {
            if (err) {
              throw err;
            }
    ...
    router.get("/budgets", async (req: Request, res: Response) => {
      try {
        // GET ALL
        const getBudgetsResponse = await xero.accountingApi.getBudgets(
          req.session.activeTenant.tenantId
        );

        // GET ONE
        const getBudgetResponse = await xero.accountingApi.getBudget(
          req.session.activeTenant.tenantId,
          getBudgetsResponse.body.budgets[0].budgetID
        );
    ...
    router.get("/banktransactions", async (req: Request, res: Response) => {
      try {
        // GET ALL
        const bankTransactionsGetResponse =
          await xero.accountingApi.getBankTransactions(
            req.session.activeTenant.tenantId
          );
        ...
        const allAccounts = await xero.accountingApi.getAccounts(
          req.session.activeTenant.tenantId
        );
        const validAccountCode = allAccounts.body.accounts.filter(
          (e) => !["NONE", "BASEXCLUDED"].includes(e.taxType)
        )[0].code;
        ...
        const useBankAccount: Account = {
          accountID: accountsResponse.body.accounts[0].accountID,
        };
        ...
        newBankTransactions.bankTransactions = [
          newBankTransaction,
          newBankTransaction,
        ];
        const bankTransactionCreateResponse =
        ...
        const newBankTransaction3: BankTransaction = {
          bankTransactionID:
            bankTransactionCreateResponse.body.bankTransactions[0]
              .bankTransactionID,
          type: BankTransaction.TypeEnum.SPEND,
          contact: useContact,
          bankAccount: useBankAccount,
        ...
        upBankTransactions.bankTransactions = [
          newBankTransaction2,
          newBankTransaction3,
        ];
        const bankTransactionUpdateOrCreateResponse =
          await xero.accountingApi.updateOrCreateBankTransactions(
            req.session.activeTenant.tenantId,
            upBankTransactions,
            false
          );
        ...
        const bankTransactionId =
          bankTransactionCreateResponse.body.bankTransactions[0]
            .bankTransactionID;
        const bankTransactionGetResponse =
          await xero.accountingApi.getBankTransaction(
            req.session.activeTenant.tenantId,
            bankTransactionId
          );
        ...
        const bankTransactionAttachmentsResponse: any =
          await xero.accountingApi.createBankTransactionAttachmentByFileName(
            req.session.activeTenant.tenantId,
            bankTransactionId,
            filename,
            readStream,
        ...
        const bankTransactionUp = Object.assign(
          {},
          bankTransactionGetResponse.body.bankTransactions[0]
        );
        ...
        const bankTransactions: BankTransactions = {
          bankTransactions: [bankTransactionUp],
        };
        const bankTransactionUpdateResponse =
        ...
        res.render("banktransactions", {
          ...
          bankTransactionsCount:
            bankTransactionsGetResponse.body.bankTransactions.length,
          createID:
            bankTransactionCreateResponse.body.bankTransactions[0]
              .bankTransactionID,
          getOneStatus:
            bankTransactionGetResponse.body.bankTransactions[0].type,
          updatedStatus:
            bankTransactionUpdateResponse.body.bankTransactions[0].status,
        });
      } catch (e) {
        console.error(e);
        res.status(res.statusCode);
    ...
    router.get("/banktranfers", async (req: Request, res: Response) => {
      try {
        // GET ALL
        const getBankTransfersResult =
          await xero.accountingApi.getBankTransfers(
            req.session.activeTenant.tenantId
          );
        ...
        const created1 = await xero.accountingApi.createAccount(
          req.session.activeTenant.tenantId,
          account1
        );
        const created2 = await xero.accountingApi.createAccount(
          req.session.activeTenant.tenantId,
          account2
        );
        ...
        res.render("banktranfers", {
          ...
          createBankTransferId:
            createBankTransfer.body.bankTransfers[0].bankTransferID,
          getBankTransferId:
            getBankTransfer.body.bankTransfers[0].bankTransferID,
        });
      } catch (e) {
        res.status(res.statusCode);
        res.render("shared/error", {
    ...
    router.get("/batchpayments", async (req: Request, res: Response) => {
      try {
        const allContacts = await xero.accountingApi.getContacts(
          req.session.activeTenant.tenantId
        );
        ...
        const batchPayments: BatchPayments = {
          // BatchPayments - the account is not typed correctly in ts BatchPayment to accept an accountID
          batchPayments: [payments, payments],
        };
        const createBatchPayment = await xero.accountingApi.createBatchPayment(
          req.session.activeTenant.tenantId,
          batchPayments
        );

        const createdBatchPaymentID =
          createBatchPayment.body.batchPayments[0].batchPaymentID;
        const createdBatchPaymentID1 =
          createBatchPayment.body.batchPayments[1].batchPaymentID;

        // GET
        const apiResponse = await xero.accountingApi.getBatchPayments(
          req.session.activeTenant.tenantId
        );

        // GET BY ID
        const getByIDResponse = await xero.accountingApi.getBatchPayment(
          req.session.activeTenant.tenantId,
          createdBatchPaymentID
        );

        // CREATE HISTORY RECORD
        const historyRecords: HistoryRecords = {
          historyRecords: [{ details: "New note added" }],
        };
        const createHistoryRecordResponse =
          await xero.accountingApi.createBatchPaymentHistoryRecord(
            req.session.activeTenant.tenantId,
            createdBatchPaymentID,
            historyRecords
          );
        ...
        // GET HISTORY RECORD
        const getHistoryRecordResponse =
          await xero.accountingApi.getBatchPaymentHistory(
            req.session.activeTenant.tenantId,
            createdBatchPaymentID
          );
        ...
        const deleteBatchPaymentResponse =
          await xero.accountingApi.deleteBatchPayment(
            req.session.activeTenant.tenantId,
            batchPaymentDelete
          );
        ...
        const batchPaymentDeleteByURLParamResponse =
          await xero.accountingApi.deleteBatchPaymentByUrlParam(
            req.session.activeTenant.tenantId,
            createdBatchPaymentID1,
            batchPaymentDeleteByURLParam
          );
        ...
        res.render("batchpayments", {
          ...
          getByBatchPaymentID: JSON.stringify(
            getByIDResponse.body.batchPayments[0],
            null,
            2
          ),
          createdHistoryRecord: JSON.stringify(
            createHistoryRecordResponse.body.historyRecords[0],
            null,
            2
          ),
          getHistoryRecord: JSON.stringify(
            getHistoryRecordResponse.body.historyRecords[0],
            null,
            2
          ),
          count: apiResponse.body.batchPayments.length,
          deletedBatchPayment: JSON.stringify(
            deleteBatchPaymentResponse.body.batchPayments[0],
            null,
            2
          ),
          deletedBatchPaymentByURLParam: JSON.stringify(
            batchPaymentDeleteByURLParamResponse.body.batchPayments[0],
            null,
            2
          ),
        });
    ...
    router.get("/brandingthemes", async (req: Request, res: Response) => {
      try {
        // GET ALL
        const getBrandingThemesResponse =
          await xero.accountingApi.getBrandingThemes(
            req.session.activeTenant.tenantId
          );

        // GET ONE
        const getBrandingThemeResponse =
          await xero.accountingApi.getBrandingTheme(
            req.session.activeTenant.tenantId,
            getBrandingThemesResponse.body.brandingThemes[0].brandingThemeID
          );
        ...
        res.render("brandingthemes", {
          ...
          authenticated: this.authenticationData(req, res),
          brandingThemesCount:
            getBrandingThemesResponse.body.brandingThemes.length,
          brandingTheme: getBrandingThemeResponse.body.brandingThemes[0].name,
          createBrandingThemePaymentService:
            "createBrandingThemePaymentServicesResponse.body.paymentServices[0].paymentServiceID",
          getBrandingThemePaymentService:
            "getBrandingThemePaymentServicesResponse.body.paymentServices[0].paymentServiceName",
        });
      } catch (e) {
        res.status(res.statusCode);
        res.render("shared/error", {
    ...
    router.get("/contacts", async (req: Request, res: Response) => {
      try {
        ...
        const contactCreateResponse = await xero.accountingApi.createContacts(
          req.session.activeTenant.tenantId,
          newContacts
        );
        ...
        await xero.accountingApi.updateOrCreateContacts(
          req.session.activeTenant.tenantId,
          updateContacts,
          false
        );
        ...
        // GET ONE
        const contactGetResponse = await xero.accountingApi.getContact(
          req.session.activeTenant.tenantId,
          contactId
        );
        ...
        const contactUpdate: Contact = {
          name: "Rick James Updated: " + Helper.getRandomNumber(1000000),
        };
        const contacts: Contacts = { contacts: [contactUpdate] };
        const contactUpdateResponse = await xero.accountingApi.updateContact(
          req.session.activeTenant.tenantId,
          contactId,
          contacts
        );
    ...
    router.get("/contactgroups", async (req: Request, res: Response) => {
      try {
        // CREATE
        const contactGroupParams: ContactGroups = {
          contactGroups: [
            { name: "Ima Contact Group" + Helper.getRandomNumber(1000000) },
          ],
        };
        const createContactGroup = await xero.accountingApi.createContactGroup(
          req.session.activeTenant.tenantId,
          contactGroupParams
        );
        ...
        // GET
        const getContactGroup = await xero.accountingApi.getContactGroup(
          req.session.activeTenant.tenantId,
        ...
        const contact1: Contact = {
          ...
          firstName: "Rick",
          lastName: "James",
          emailAddress: `foo+${num}@example.com`,
        };
        const newContacts: Contacts = new Contacts();
        newContacts.contacts = [contact1];
        const contactCreateResponse = await xero.accountingApi.createContacts(
        ...
        const updatedContactGroup =
          await xero.accountingApi.createContactGroupContacts(
            req.session.activeTenant.tenantId,
            contactGroup.contactGroupID,
            updatedContactGroupParams
          );

        // DELETE
        const deletedContactGroupContact =
        ...
        // GET ALL
        const allContactGroups = await xero.accountingApi.getContactGroups(
          req.session.activeTenant.tenantId
        );
        ...
        res.render("contactgroups", {
          ...
          updatedContactGroupContactID:
            updatedContactGroup.body.contacts[0].contactID,
          deletedContactGroupContact: deleted
            ? `${createdContact.contactID} removed from contact group`
            : "failed to delete",
    ...
    router.get("/creditnotes", async (req: Request, res: Response) => {
      try {
        //GET ALL
        const getCreditNotesResponse = await xero.accountingApi.getCreditNotes(
          req.session.activeTenant.tenantId
        );
        ...
        const createCreditNotesResponse =
        ...
        const updateCreditNoteResponse =
          await xero.accountingApi.updateCreditNote(
            req.session.activeTenant.tenantId,
        ...
        const historyRecords: HistoryRecords = {
          historyRecords: [
            {
              details:
                "This is a history record " + Helper.getRandomNumber(1000),
            },
          ],
        };

        const createCreditNoteHistoryResponse =
          await xero.accountingApi.createCreditNoteHistory(
            req.session.activeTenant.tenantId,
            createCreditNotesResponse.body.creditNotes[0].creditNoteID,
            historyRecords
          );
        ...
        const createCreditNoteAllocationResponse =
        ...
        const createCreditNoteAttachmentByFileNameResponse =
          await xero.accountingApi.createCreditNoteAttachmentByFileName(
            req.session.activeTenant.tenantId,
            createCreditNotesResponse.body.creditNotes[0].creditNoteID,
            filename,
            readStream,
        ...
        // GET CREDIT NOTE
        const getCreditNoteResponse = await xero.accountingApi.getCreditNote(
          req.session.activeTenant.tenantId,
          createCreditNotesResponse.body.creditNotes[0].creditNoteID
        );

        // GET CREDIT NOTE HISTORY
        const getCreditNoteHistoryResponse =
          await xero.accountingApi.getCreditNoteHistory(
            req.session.activeTenant.tenantId,
            createCreditNotesResponse.body.creditNotes[0].creditNoteID
          );

        // GET CREDIT NOTE ATTACHMENTS
        const getCreditNoteAttachmentsResponse =
          await xero.accountingApi.getCreditNoteAttachments(
            req.session.activeTenant.tenantId,
            createCreditNotesResponse.body.creditNotes[0].creditNoteID
          );

        // GET CREDIT NOTE ATTACHMENT BY ID
        const getCreditNoteAttachmentByIdResponse =
          await xero.accountingApi.getCreditNoteAttachmentById(
            req.session.activeTenant.tenantId,
            createCreditNotesResponse.body.creditNotes[0].creditNoteID,
        ...
        const getCreditNoteAttachmentByFileNameResponse =
          await xero.accountingApi.getCreditNoteAttachmentByFileName(
            req.session.activeTenant.tenantId,
            createCreditNotesResponse.body.creditNotes[0].creditNoteID,
        ...
        const getCreditNoteAsPdfResponse =
          await xero.accountingApi.getCreditNoteAsPdf(
            req.session.activeTenant.tenantId,
            createCreditNotesResponse.body.creditNotes[0].creditNoteID
          );
        res.render("creditnotes", {
          ...
          createHistoryRecord:
            createCreditNoteHistoryResponse.body.historyRecords[0].details,
          createAllocation:
            createCreditNoteAllocationResponse.body.allocations[0].amount,
          createAttachmentByFileName:
            createCreditNoteAttachmentByFileNameResponse.body.attachments[0]
              .attachmentID,
          getOne: getCreditNoteResponse.body.creditNotes[0].contact.contactID,
          historyRecords:
            getCreditNoteHistoryResponse.body.historyRecords.length,
          attachmentsCount:
            getCreditNoteAttachmentsResponse.body.attachments.length,
    ...
    router.get("/currencies", async (req: Request, res: Response) => {
      try {
        //GET ALL
        const apiResponse = await xero.accountingApi.getCurrencies(
          req.session.activeTenant.tenantId
        );
    ...
    router.get("/employees", async (req: Request, res: Response) => {
      try {
        //GET ALL
        const getEmployeesResponse = await xero.accountingApi.getEmployees(
          req.session.activeTenant.tenantId
        );
        ...
        const createEmployeesResponse =
          await xero.accountingApi.createEmployees(
            req.session.activeTenant.tenantId,
            employees
          );

        // GET ONE
        const getEmployeeResponse = await xero.accountingApi.getEmployee(
          req.session.activeTenant.tenantId,
          createEmployeesResponse.body.employees[0].employeeID
        );
        ...
        const updateEmployeeResponse =
          await xero.accountingApi.updateOrCreateEmployees(
            req.session.activeTenant.tenantId,
            updatedEmployees
          );
        res.render("employees", {
          ...
          updatedEmployeeId:
            updateEmployeeResponse.body.employees[0].employeeID,
        });
      } catch (e) {
        res.status(res.statusCode);
        res.render("shared/error", {
    ...
    router.get("/invoicereminders", async (req: Request, res: Response) => {
      try {
        //GET ALL
        const apiResponse = await xero.accountingApi.getInvoiceReminders(
          req.session.activeTenant.tenantId
        );
    ...
    router.get("/invoice-attachments", async (req: Request, res: Response) => {
      try {
        ...
        const attachments = await xero.accountingApi.getInvoiceAttachments(
          req.session.activeTenant.tenantId,
    ...
    router.get("/bills", async (req: Request, res: Response) => {
      try {
        ...
        const invoice1: Invoice = {
          ...
          invoiceNumber: `Zorro:${Helper.getRandomNumber(1000000)}`,
          reference: `Testy:${Helper.getRandomNumber(1000000)}`,
        ...
        // UPDATE
        const newReference = {
          reference: `NEW-beference:${Helper.getRandomNumber(1000000)}`,
        };
    ...
    router.get("/invoices", async (req: Request, res: Response) => {
      try {
        ...
        // UPDATE
        const newReference = {
          reference: `NEW-REF:${Helper.getRandomNumber(1000000)}`,
        };
    ...
    router.get("/invoice-as-pdf", async (req: Request, res: Response) => {
      try {
        ...
        res.setHeader(
          "Content-Disposition",
          "attachment; filename=invoice-as-pdf.pdf"
        );
    ...
    router.get("/email-invoice", async (req: Request, res: Response) => {
      try {
        ...
        // SEND Email
        const apiResponse = await xero.accountingApi.emailInvoice(
          req.session.activeTenant.tenantId,
          <string>invoiceID,
          {}
        );
    ...
    router.get("/items", async (req: Request, res: Response) => {
      ...
      try {
        // GET ALL
        const itemsGetResponse = await xero.accountingApi.getItems(
          req.session.activeTenant.tenantId
        );
        ...
        const itemCreateResponse = await xero.accountingApi.createItems(
          req.session.activeTenant.tenantId,
          newItems
        );
        ...
        // GET ONE
        const itemGetResponse = await xero.accountingApi.getItem(
          req.session.activeTenant.tenantId,
          itemsGetResponse.body.items[0].itemID
        );
        ...
        const itemUpdate: Item = {
          code: "Foo" + Helper.getRandomNumber(1000000),
          name: "Bar - updated",
          inventoryAssetAccountCode: item1.inventoryAssetAccountCode,
        };
        const items: Items = { items: [itemUpdate] };
        const itemUpdateResponse = await xero.accountingApi.updateItem(
          req.session.activeTenant.tenantId,
          itemId,
          items
        );
        ...
        // DELETE
        const itemDeleteResponse = await xero.accountingApi.deleteItem(
          req.session.activeTenant.tenantId,
          itemId
        );
    ...
    router.get("/journals", async (req: Request, res: Response) => {
      try {
        //GET ALL
        const getAllResponse = await xero.accountingApi.getJournals(
          req.session.activeTenant.tenantId
        );
        const journalNumber = getAllResponse.body.journals[0].journalNumber;
        const getByNumberResponse = await xero.accountingApi.getJournalByNumber(
          req.session.activeTenant.tenantId,
          journalNumber
        );
        res.render("journals", {
    ...
    router.get("/linked-transactions", async (req: Request, res: Response) => {
      try {
        //GET ALL
        const getLinkedTransactionsResponse =
          await xero.accountingApi.getLinkedTransactions(
            req.session.activeTenant.tenantId
          );
        ...
        const createInvoicesResponse = await xero.accountingApi.createInvoices(
          req.session.activeTenant.tenantId,
          invoices
        );
        ...
        const newLinkedTransaction: LinkedTransaction = {
          sourceTransactionID:
            createInvoicesResponse.body.invoices[0].invoiceID,
          sourceLineItemID:
            createInvoicesResponse.body.invoices[0].lineItems[0].lineItemID,
        };

        const createLinkedTransactionResponse =
          await xero.accountingApi.createLinkedTransaction(
            req.session.activeTenant.tenantId,
            newLinkedTransaction
          );

        // GET ONE
        const getLinkedTransactionResponse =
          await xero.accountingApi.getLinkedTransaction(
            req.session.activeTenant.tenantId,
            createLinkedTransactionResponse.body.linkedTransactions[0]
              .linkedTransactionID
          );
          ...
          linkedTransactions: [
            {
              linkedTransactionID:
                createLinkedTransactionResponse.body.linkedTransactions[0]
                  .linkedTransactionID,
              contactID:
                createInvoicesResponse.body.invoices[1].contact.contactID,
              targetTransactionID:
                createInvoicesResponse.body.invoices[1].invoiceID,
              targetLineItemID:
                createInvoicesResponse.body.invoices[1].lineItems[0].lineItemID,
            },
          ],
        };
        const updateLinkedTransactionResponse =
          await xero.accountingApi.updateLinkedTransaction(
            req.session.activeTenant.tenantId,
            createLinkedTransactionResponse.body.linkedTransactions[0]
              .linkedTransactionID,
        ...
        // DELETE
        const deleteLinkedTransactionResponse =
          await xero.accountingApi.deleteLinkedTransaction(
            req.session.activeTenant.tenantId,
            createLinkedTransactionResponse.body.linkedTransactions[0]
              .linkedTransactionID
          );
    ...
    router.get("/manualjournals", async (req: Request, res: Response) => {
      try {
        //GET ALL
        const getManualJournalsResponse =
          await xero.accountingApi.getManualJournals(
            req.session.activeTenant.tenantId
          );
        // CREATE
          ...
          manualJournals: [
            {
              ...
              narration:
                "Accrued expenses - prepaid insurance adjustment for March 2020",
              journalLines: [
                {
                  lineAmount: 55.0,
                  accountCode: "433",
        ...
        const createManualJournalResponse =
          await xero.accountingApi.createManualJournals(
            req.session.activeTenant.tenantId,
            manualJournals
          );
        ...
        const pathToUpload = path.resolve(
          __dirname,
          "../public/images/xero-dev.png"
        ); // determine the path to your file

        const body = fs.createReadStream(pathToUpload); // {fs.ReadStream} read the file
        const contentType = mime.lookup(fileName);
        const journalId =
          createManualJournalResponse.body.manualJournals[0].manualJournalID;
        const createManualJournalAttachmentByFileNameResponse: any =
          await xero.accountingApi.createManualJournalAttachmentByFileName(
            req.session.activeTenant.tenantId,
            journalId,
            fileName,
            body,
        ...
        // GET ONE
        const getManualJournalResponse =
          await xero.accountingApi.getManualJournal(
            req.session.activeTenant.tenantId,
            journalId
          );

        // GET MANUAL JOURNAL ATTACHMENTS
        const getManualJournalAttachmentsResponse =
          await xero.accountingApi.getManualJournalAttachments(
            req.session.activeTenant.tenantId,
            journalId
          );

        // GET MANUAL JOURNAL ATTACHMENT BY FILENAME
        const getManualJournalAttachmentByFileNameResponse =
          await xero.accountingApi.getManualJournalAttachmentByFileName(
            req.session.activeTenant.tenantId,
            journalId,
            fileName,
            contentType
        ...
        const getManualJournalAttachmentByIdResponse =
          await xero.accountingApi.getManualJournalAttachmentById(
            req.session.activeTenant.tenantId,
            journalId,
            getManualJournalResponse.body.manualJournals[0].attachments[0]
              .attachmentID,
            contentType
          );
        ...
        const updateManualJournalResponse =
          await xero.accountingApi.updateManualJournal(
            req.session.activeTenant.tenantId,
            journalId,
            manualJournals
          );
        ...
        res.render("manualjournals", {
          ...
          create:
            createManualJournalResponse.body.manualJournals[0].manualJournalID,
          mjAttachmentByFileName:
            createManualJournalAttachmentByFileNameResponse.body,
          getMJ: getManualJournalResponse.body.manualJournals[0].narration,
          getMJAttachments: getManualJournalAttachmentsResponse.body,
          getMJAttachmentByFileName:
            getManualJournalAttachmentByFileNameResponse.body,
          getMJAttachmentById: getManualJournalAttachmentByIdResponse.body,
          updateMJ:
            updateManualJournalResponse.body.manualJournals[0].journalLines[0]
              .description,
        });
      } catch (e) {
        res.status(res.statusCode);
    ...
    router.get("/overpayments", async (req: Request, res: Response) => {
      try {
        //GET ALL
        const getOverpaymentsResponse =
          await xero.accountingApi.getOverpayments(
            req.session.activeTenant.tenantId
          );
        ...
        const newBankTransaction: BankTransaction = {
          ...
          lineItems: [
            {
              description: "Forgot to cancel the auto payment",
              lineAmount: 40.0,
            },
          ],
          bankAccount: {
        ...
        const overpaymentAllocationResponse =
          await xero.accountingApi.createOverpaymentAllocations(
            req.session.activeTenant.tenantId,
            newBankTransactionResponse.body.bankTransactions[0].overpaymentID,
            newAllocations
          );

        // GET ONE
        const getOverpaymentResponse = await xero.accountingApi.getOverpayment(
          req.session.activeTenant.tenantId,
          newBankTransactionResponse.body.bankTransactions[0].overpaymentID
        );
    ...
    router.get("/payments", async (req: Request, res: Response) => {
      try {
        //GET ALL
        const getPaymentsResponse = await xero.accountingApi.getPayments(
          req.session.activeTenant.tenantId
        );
        ...
        const createPaymentResponse = await xero.accountingApi.createPayments(
          req.session.activeTenant.tenantId,
          payments
        );

        // GET ONE
        const getPaymentResponse = await xero.accountingApi.getPayment(
          req.session.activeTenant.tenantId,
    ...
    router.get("/paymentservices", async (req: Request, res: Response) => {
      ...
      try {
        //GET ALL
        const getPaymentServicesResponse =
          await xero.accountingApi.getPaymentServices(
            req.session.activeTenant.tenantId
          );
        ...
        const paymentServices: PaymentServices = {
          paymentServices: [
            {
              paymentServiceName: `PayUpNow ${Helper.getRandomNumber(1000)}`,
              paymentServiceUrl:
                "https://www.payupnow.com/?invoiceNo=[INVOICENUMBER]&currency=[CURRENCY]&amount=[AMOUNTDUE]&shortCode=[SHORTCODE]",
              payNowText: "Time To Pay",
            },
          ],
        };
        const createPaymentServiceResponse =
          await xero.accountingApi.createPaymentService(
            req.session.activeTenant.tenantId,
            paymentServices
          );
        ...
        res.render("paymentservices", {
          ...
          create:
            createPaymentServiceResponse.body.paymentServices[0]
              .paymentServiceID,
        });
      } catch (e) {
        res.status(res.statusCode);
    ...
    router.get("/prepayments", async (req: Request, res: Response) => {
      try {
        //GET ALL
        const getPrepaymentsResponse = await xero.accountingApi.getPrepayments(
          req.session.activeTenant.tenantId
        );
        ...
        const newAllocations: Allocations = new Allocations();
        newAllocations.allocations = [allocation];
        const prepaymentAllocationResponse =
          await xero.accountingApi.createPrepaymentAllocations(
            req.session.activeTenant.tenantId,
            newBankTransactionResponse.body.bankTransactions[0].prepaymentID,
            newAllocations
          );
        ...
        // GET ONE
        const getPrepaymentResponse = await xero.accountingApi.getPrepayment(
          req.session.activeTenant.tenantId,
          newBankTransactionResponse.body.bankTransactions[0].prepaymentID
        );
        ...
        res.render("prepayments", {
          ...
          remainingCredit:
            getPrepaymentResponse.body.prepayments[0].remainingCredit,
        });
      } catch (e) {
        res.status(res.statusCode);
        res.render("shared/error", {
    ...
    router.get("/purchaseorders", async (req: Request, res: Response) => {
      try {
        //GET ALL
        const getPurchaseOrdersResponse =
          await xero.accountingApi.getPurchaseOrders(
            req.session.activeTenant.tenantId
          );
        ...
        const createPurchaseOrderResponse =
          await xero.accountingApi.createPurchaseOrders(
            req.session.activeTenant.tenantId,
            purchaseOrders
          );

        // GET ONE
        const getPurchaseOrderResponse =
          await xero.accountingApi.getPurchaseOrder(
            req.session.activeTenant.tenantId,
        ...
        res.render("purchaseorders", {
          ...
          create:
            createPurchaseOrderResponse.body.purchaseOrders[0].purchaseOrderID,
          get: getPurchaseOrderResponse.body.purchaseOrders[0].lineItems[0]
            .description,
          update:
            updatePurchaseOrderResponse.body.purchaseOrders[0]
    ...
    router.get(
      ...
      async (req: Request, res: Response) => {
        try {
          ...
          res.setHeader(
            "Content-Disposition",
            "attachment; filename=purchase-order-as-pdf.pdf"
          );
          res.contentType("application/pdf");
          res.send(getAsPdf.body);
        } catch (e) {
    ...
    router.get("/receipts", async (req: Request, res: Response) => {
      try {
        //GET ALL
        const getReceiptsResponse = await xero.accountingApi.getReceipts(
          req.session.activeTenant.tenantId
        );
        ...
        const contactID = getContactsResponse.body.contacts[0].contactID;
        const getusersResponse = await xero.accountingApi.getUsers(
          req.session.activeTenant.tenantId
        );
        ...
        // CREATE
        const createReceiptResponse = await xero.accountingApi.createReceipt(
          req.session.activeTenant.tenantId,
          receipts
        );

        // GET ONE
        const getReceiptResponse = await xero.accountingApi.getReceipt(
          req.session.activeTenant.tenantId,
          createReceiptResponse.body.receipts[0].receiptID
        );
        const updatedReceipts: Receipts = receipts;
        updatedReceipts.receipts[0].lineItems[0].description =
          "UPDATED - Foobar";

        // UPDATE
        const updateReceiptResponse = await xero.accountingApi.updateReceipt(
          req.session.activeTenant.tenantId,
          getReceiptResponse.body.receipts[0].receiptID,
          updatedReceipts
        );
        ...
        res.render("receipts", {
          ...
          update:
            updateReceiptResponse.body.receipts[0].lineItems[0].description,
        });
      } catch (e) {
        res.status(res.statusCode);
        res.render("shared/error", {
    ...
    router.get("/repeating-invoices", async (req: Request, res: Response) => {
      try {
        //GET ALL
        const getRepeatingInvoices =
          await xero.accountingApi.getRepeatingInvoices(
            req.session.activeTenant.tenantId
          );

        // GET BY ID
        const getRepeatingInvoice =
          await xero.accountingApi.getRepeatingInvoice(
            req.session.activeTenant.tenantId,
            getRepeatingInvoices.body.repeatingInvoices[0].repeatingInvoiceID
          );
        ...
        const createRepeatingInvoices =
          await xero.accountingApi.createRepeatingInvoices(
            req.session.activeTenant.tenantId,
            repeatingInvoices
          );

        const repeatingInvoiceToUpdate: RepeatingInvoice =
          createRepeatingInvoices.body.repeatingInvoices[0];

        repeatingInvoiceToUpdate.status = RepeatingInvoice.StatusEnum.DELETED;

        const updateRepeatingInvoices =
          await xero.accountingApi.updateRepeatingInvoice(
            req.session.activeTenant.tenantId,
            repeatingInvoiceToUpdate.repeatingInvoiceID,
            { repeatingInvoices: [repeatingInvoiceToUpdate] }
          );
    ...
    router.get("/reports", async (req: Request, res: Response) => {
      try {
        ...
        const getAgedPayablesByContactResponse =
          await xero.accountingApi.getReportAgedPayablesByContact(
            req.session.activeTenant.tenantId,
            apbcContactID,
            apbcDate,
            apbcFromDate,
        ...
        const getAgedReceivablesByContactResponse =
          await xero.accountingApi.getReportAgedReceivablesByContact(
            req.session.activeTenant.tenantId,
            arbcContactID,
            arbcDate,
            arbcFromDate,
        ...
        const getReportBankSummaryResponse =
          await xero.accountingApi.getReportBankSummary(
            req.session.activeTenant.tenantId,
            banksumFromDate,
            banksumToDate
          );
        ...
        const bsPeriods = 6;
        const bsTimeframe = 3;
        const getBudgetSummaryResponse =
          await xero.accountingApi.getReportBudgetSummary(
            req.session.activeTenant.tenantId,
            bsDate,
            bsPeriods,
            bsTimeframe
        ...
        const getExecutiveSummaryResponse =
          await xero.accountingApi.getReportExecutiveSummary(
            req.session.activeTenant.tenantId,
            esDate
          );

        // GET OTHER REPORT - FOR NEW ZEALAND/AUS ORGS GST/BAS
        const getReportsList = await xero.accountingApi.getReportsList(
          req.session.activeTenant.tenantId
        );
        const getReportFromId = await xero.accountingApi.getReportFromId(
          req.session.activeTenant.tenantId,
          getReportsList.body.reports[0].reportID
        );
        ...
        const getProfitAndLossResponse =
          await xero.accountingApi.getReportProfitAndLoss(
            ...
            plPeriods,
            plTimeframe,
            plTrackingCategoryID,
            plTrackingOptionID,
            plTrackingCategoryID2,
            plTrackingOptionID2,
            plStandardLayout,
            plPaymentsOnly
          );
        ...
        const getTrialBalanceResponse =
          await xero.accountingApi.getReportTrialBalance(
            req.session.activeTenant.tenantId,
            tbDate,
            tbPaymentsOnly
          );
        ...
        res.render("reports", {
          consentUrl: await xero.buildConsentUrl(),
          authenticated: this.authenticationData(req, res),
          bankSummaryReportTitle:
            getReportBankSummaryResponse.body.reports[0].reportTitles.join(" "),
          tenNinetyNineReportTitle:
            "`${getTenNinetyNineResponse.body.reports[0].reportName} ${getTenNinetyNineResponse.body.reports[0].reportDate}`",
    ...
    router.get("/taxrates", async (req: Request, res: Response) => {
      try {
        //GET ALL
        const getAllResponse = await xero.accountingApi.getTaxRates(
          req.session.activeTenant.tenantId
        );
        ...
        // CREATE
        const createResponse = await xero.accountingApi.createTaxRates(
          req.session.activeTenant.tenantId,
          taxRates
        );
    ...
    router.get("/trackingcategories", async (req: Request, res: Response) => {
      try {
        // GET ALL
        const getAllResponse = await xero.accountingApi.getTrackingCategories(
          req.session.activeTenant.tenantId
        );
        ...
        const createCategoryResponse =
          await xero.accountingApi.createTrackingCategory(
            req.session.activeTenant.tenantId,
            trackingCategory
          );
        await xero.accountingApi.createTrackingOptions(
          req.session.activeTenant.tenantId,
          createCategoryResponse.body.trackingCategories[0].trackingCategoryID,
          trackingCategoryOption
        );

        // GET ONE
        const getOneResponse = await xero.accountingApi.getTrackingCategory(
          req.session.activeTenant.tenantId,
          createCategoryResponse.body.trackingCategories[0].trackingCategoryID
        );

        // UPDATE
        const updateResponse = await xero.accountingApi.updateTrackingCategory(
          req.session.activeTenant.tenantId,
          getOneResponse.body.trackingCategories[0].trackingCategoryID,
          {
            name: `${getOneResponse.body.trackingCategories[0].name} - updated`,
          }
        );

        // DELETE
        const deleteResponse = await xero.accountingApi.deleteTrackingCategory(
          req.session.activeTenant.tenantId,
          createCategoryResponse.body.trackingCategories[0].trackingCategoryID
        );
        ...
        res.render("trackingcategories", {
          ...
          created:
            createCategoryResponse.body.trackingCategories[0]
              .trackingCategoryID,
    ...
    router.get("/users", async (req: Request, res: Response) => {
      try {
        //GET ALL
        const getAllUsers = await xero.accountingApi.getUsers(
          req.session.activeTenant.tenantId
        );

        // GET ONE USER
        const getUser = await xero.accountingApi.getUser(
          req.session.activeTenant.tenantId,
          getAllUsers.body.users[0].userID
        );
        res.render("users", {
          consentUrl: await xero.buildConsentUrl(),
          authenticated: this.authenticationData(req, res),
    ...
    router.get("/quotes", async (req: Request, res: Response) => {
      try {
        ...
        const addQuoteAttachment =
          await xero.accountingApi.createQuoteAttachmentByFileName(
            req.session.activeTenant.tenantId,
            quoteId,
            filename,
            readStream,
    ...
    router.get("/assets", async (req: Request, res: Response) => {
      try {
        // GET ASSET SETTINGS
        const getAssetSettings = await xero.assetApi.getAssetSettings(
          req.session.activeTenant.tenantId
        );

        // GET ASSETTYPES
        const getAssetTypes = await xero.assetApi.getAssetTypes(
          req.session.activeTenant.tenantId
        );
        ...
        const createAsset = await xero.assetApi.createAsset(
          req.session.activeTenant.tenantId,
          asset
        );

        // GET ASSET
        const getAsset = await xero.assetApi.getAssetById(
          req.session.activeTenant.tenantId,
          createAsset.body.assetId
        );

        // GET ASSETS
        const getAssets = await xero.assetApi.getAssets(
          req.session.activeTenant.tenantId,
    ...
    router.get("/projects", async (req: Request, res: Response) => {
      try {
        //GET ALL
        const getAllResponse = await xero.projectApi.getProjects(
          req.session.activeTenant.tenantId
        );

        //GET MULTIPLE SPECIFIED
        const getMultipleSpecifiedResponse = await xero.projectApi.getProjects(
          req.session.activeTenant.tenantId,
          [
            getAllResponse.body.items[0].projectId,
            getAllResponse.body.items[1].projectId,
          ]
        );
        ...
        const createResponse = await xero.projectApi.createProject(
          req.session.activeTenant.tenantId,
          newProject
        );
        ...
        // GET ONE
        const getResponse = await xero.projectApi.getProject(
          req.session.activeTenant.tenantId,
          createResponse.body.projectId
        );
        ...
        const updateProject: ProjectCreateOrUpdate = {
          ...
          estimateAmount: 350.0,
        };

        const updateResponse = await xero.projectApi.updateProject(
          req.session.activeTenant.tenantId,
          createResponse.body.projectId,
          updateProject
        );
        ...
        const patchResponse = await xero.projectApi.patchProject(
          req.session.activeTenant.tenantId,
          createResponse.body.projectId,
          patch
        );
    ...
    router.get("/project-users", async (req: Request, res: Response) => {
      try {
        // GET PROJECT USERS
        const getProjectUsersResponse = await xero.projectApi.getProjectUsers(
          req.session.activeTenant.tenantId
        );
    ...
    router.get("/tasks", async (req: Request, res: Response) => {
      try {
        ...
        const projectsResponse = await xero.projectApi.getProjects(
          req.session.activeTenant.tenantId
        );

        const getTasksResponse = await xero.projectApi.getTasks(
          req.session.activeTenant.tenantId,
          projectsResponse.body.items[0].projectId
        );
        // CREATE
        const rate: Amount = {
        ...
        const createTaskResponse = await xero.projectApi.createTask(
          req.session.activeTenant.tenantId,
          projectsResponse.body.items[0].projectId,
          newTask
        );
        // GET ONE
        const getTaskResponse = await xero.projectApi.getTask(
          req.session.activeTenant.tenantId,
          projectsResponse.body.items[0].projectId,
          getTasksResponse.body.items[0].taskId
        );
        // UPDATE
        ...
        const updateTaskResponse = await xero.projectApi.updateTask(
          req.session.activeTenant.tenantId,
          projectsResponse.body.items[0].projectId,
          getTasksResponse.body.items[0].taskId,
          updateTask
        ...
        // DELETE
        const deleteTaskResponse = await xero.projectApi.deleteTask(
          req.session.activeTenant.tenantId,
          projectsResponse.body.items[0].projectId,
          getTasksResponse.body.items[0].taskId
        );
    ...
    router.get("/time", async (req: Request, res: Response) => {
      try {
        ...
        const getTimeEntriesResponse = await xero.projectApi.getTimeEntries(
          req.session.activeTenant.tenantId,
          projectsResponse.body.items[0].projectId
        );
        ...
        const createTimeEntryResponse = await xero.projectApi.createTimeEntry(
          req.session.activeTenant.tenantId,
          projectsResponse.body.items[0].projectId,
          timeEntry
        );
        ...
        // GET ONE
        const getTimeEntryResponse = await xero.projectApi.getTimeEntry(
          req.session.activeTenant.tenantId,
          projectsResponse.body.items[0].projectId,
          createTimeEntryResponse.body.timeEntryId
        );

        // UPDATE
        timeEntry.description = "time it takes to become an expert - edited";
        const updateTimeEntryResponse = await xero.projectApi.updateTimeEntry(
          req.session.activeTenant.tenantId,
          projectsResponse.body.items[0].projectId,
          createTimeEntryResponse.body.timeEntryId,
          timeEntry
        ...
        // DELETE
        const deleteTimeEntryResponse = await xero.projectApi.deleteTimeEntry(
          req.session.activeTenant.tenantId,
          projectsResponse.body.items[0].projectId,
          createTimeEntryResponse.body.timeEntryId
        );
    ...
    router.get("/payroll-au-employees", async (req: Request, res: Response) => {
      try {
        ...
        const createEmployee = await xero.payrollAUApi.createEmployee(
          req.session.activeTenant.tenantId,
          [employee]
        );

        const getEmployees = await xero.payrollAUApi.getEmployees(
          req.session.activeTenant.tenantId
        );
        ...
        const updateEmployee = await xero.payrollAUApi.updateEmployee(
          req.session.activeTenant.tenantId,
          getEmployees.body.employees[0].employeeID,
          [updatedEmployee]
        );
        ...
        console.log(
          "Are you using an Australia Org with the Payroll settings completed? (https://payroll.xero.com/Dashboard/Details)"
        );
        res.status(res.statusCode);
        res.render("shared/error", {
          consentUrl: await xero.buildConsentUrl(),
    ...
    router.get("/leave-application", async (req: Request, res: Response) => {
      try {
        const leaveItems = await xero.payrollAUApi.getLeaveApplications(
          req.session.activeTenant.tenantId
        );
    ...
    router.get("/pay-item", async (req: Request, res: Response) => {
      try {
        const payItems = await xero.payrollAUApi.getPayItems(
          req.session.activeTenant.tenantId
        );
    ...
    router.get("/pay-run", async (req: Request, res: Response) => {
      try {
        const payRuns = await xero.payrollAUApi.getPayRuns(
          req.session.activeTenant.tenantId
        );
    ...
    router.get("/payroll-calendar", async (req: Request, res: Response) => {
      try {
        ...
        const getPayrollCalendars = await xero.payrollAUApi.getPayrollCalendars(
          req.session.activeTenant.tenantId
        );
    ...
    router.get("/superfund", async (req: Request, res: Response) => {
      try {
        const getSuperfunds = await xero.payrollAUApi.getSuperfunds(
          req.session.activeTenant.tenantId
        );
    ...
    router.get("/payroll-au-settings", async (req: Request, res: Response) => {
      try {
        const getPayrollSettingsResponse = await xero.payrollAUApi.getSettings(
          req.session.activeTenant.tenantId
        );
    ...
    router.get("/bankfeed-connections", async (req: Request, res: Response) => {
      try {
        const getBankfeedsResponse = await xero.bankFeedsApi.getFeedConnections(
          req.session.activeTenant.tenantId
        );
        ...
        const createBankfeedResponse =
          await xero.bankFeedsApi.createFeedConnections(
            req.session.activeTenant.tenantId,
            feedConnections
          );
        ...
        const getBankfeedResponse = await xero.bankFeedsApi.getFeedConnection(
          req.session.activeTenant.tenantId,
          createBankfeedResponse.body.items[0].id
        );
        ...
        const deleteBankfeedResponse =
          await xero.bankFeedsApi.deleteFeedConnections(
            req.session.activeTenant.tenantId,
            deleteConnection
          );
        ...
        console.log(
          "Do you have XeroAPI permissions to work with this endpoint? (https://developer.xero.com/documentation/bank-feeds-api/overview)"
        );
        res.status(res.statusCode);
        res.render("shared/error", {
          consentUrl: await xero.buildConsentUrl(),
    ...
    router.get("/bankfeed-statements", async (req: Request, res: Response) => {
      try {
        const getStatementsResponse = await xero.bankFeedsApi.getStatements(
          req.session.activeTenant.tenantId
        );
        ...
        const createStatementResponse =
          await xero.bankFeedsApi.createStatements(
            req.session.activeTenant.tenantId,
            statements
          );

        const getStatementResponse = await xero.bankFeedsApi.getStatement(
          req.session.activeTenant.tenantId,
          createStatementResponse.body.items[0].id
        );
    ...
    router.get("/payroll-uk-employees", async (req: Request, res: Response) => {
      try {
        const getEmployeesResponse = await xero.payrollUKApi.getEmployees(
          req.session.activeTenant.tenantId
        );
        ...
        const createEmployeeResponse = await xero.payrollUKApi.createEmployee(
          req.session.activeTenant.tenantId,
          employee
        );

        const getEmployeeResponse = await xero.payrollUKApi.getEmployee(
          req.session.activeTenant.tenantId,
          createEmployeeResponse.body.employee.employeeID
        );

        const updatedEmployee = employee;
        updatedEmployee.email = "thetelltaleheart@gmail.com";

        const updateEmployeeResponse = await xero.payrollUKApi.updateEmployee(
          req.session.activeTenant.tenantId,
          createEmployeeResponse.body.employee.employeeID,
          updatedEmployee
        );
    ...
    router.get("/employees-tax", async (req: Request, res: Response) => {
      try {
        ...
        const getEmployeeTaxResponse = await xero.payrollUKApi.getEmployeeTax(
          req.session.activeTenant.tenantId,
          getEmployeesResponse.body.employees[0].employeeID
        );
    ...
    router.get("/employees-leave", async (req: Request, res: Response) => {
      try {
        ...
        const response = await xero.payrollUKApi.getEmployeeLeaves(
          req.session.activeTenant.tenantId,
          getEmployeesResponse.body.employees[0].employeeID
        );
    ...
    router.get(
      ...
      async (req: Request, res: Response) => {
        try {
          ...
          const response = await xero.payrollUKApi.getEmployeeLeaveBalances(
            req.session.activeTenant.tenantId,
            getEmployeesResponse.body.employees[0].employeeID
          );
    ...
    router.get(
      ...
      async (req: Request, res: Response) => {
        try {
          ...
          const response =
            await xero.payrollUKApi.getEmployeeStatutoryLeaveBalances(
              req.session.activeTenant.tenantId,
              getEmployeesResponse.body.employees[0].employeeID
            );
    ...
    router.get(
      ...
      async (req: Request, res: Response) => {
        try {
          ...
          const response = await xero.payrollUKApi.getStatutoryLeaveSummary(
            req.session.activeTenant.tenantId,
            getEmployeesResponse.body.employees[0].employeeID
          );
    ...
    router.get(
      ...
      async (req: Request, res: Response) => {
        try {
          ...
          const response = await xero.payrollUKApi.getEmployeeLeavePeriods(
            req.session.activeTenant.tenantId,
            getEmployeesResponse.body.employees[0].employeeID,
            "2018-06-15",
            "2020-06-15"
    ...
    router.get(
      ...
      async (req: Request, res: Response) => {
        try {
          ...
          const response = await xero.payrollUKApi.getEmployeeLeaveTypes(
            req.session.activeTenant.tenantId,
            getEmployeesResponse.body.employees[0].employeeID
          );
    ...
    router.get(
      ...
      async (req: Request, res: Response) => {
        try {
          ...
          const response = await xero.payrollUKApi.getEmployeePayTemplate(
            req.session.activeTenant.tenantId,
            getEmployeesResponse.body.employees[0].employeeID
          );
    ...
    router.get("/employer-pensions", async (req: Request, res: Response) => {
      try {
        const response = await xero.payrollUKApi.getBenefits(
          req.session.activeTenant.tenantId
        );
    ...
    router.get("/deductions", async (req: Request, res: Response) => {
      try {
        const response = await xero.payrollUKApi.getDeductions(
          req.session.activeTenant.tenantId
        );
    ...
    router.get("/earnings-orders", async (req: Request, res: Response) => {
      try {
        const response = await xero.payrollUKApi.getEarningsOrders(
          req.session.activeTenant.tenantId
        );
    ...
    router.get("/earnings-rates", async (req: Request, res: Response) => {
      try {
        const response = await xero.payrollUKApi.getEarningsRates(
          req.session.activeTenant.tenantId
        );
    ...
    router.get("/leave-types", async (req: Request, res: Response) => {
      try {
        const response = await xero.payrollUKApi.getLeaveTypes(
          req.session.activeTenant.tenantId
        );
    ...
    router.get("/reimbursements", async (req: Request, res: Response) => {
      try {
        const response = await xero.payrollUKApi.getReimbursements(
          req.session.activeTenant.tenantId
        );
    ...
    router.get("/timesheets", async (req: Request, res: Response) => {
      try {
        const response = await xero.payrollUKApi.getTimesheets(
          req.session.activeTenant.tenantId
        );
    ...
    router.get("/payment-methods", async (req: Request, res: Response) => {
      try {
        ...
        const response = await xero.payrollUKApi.getEmployeePaymentMethod(
          req.session.activeTenant.tenantId,
          getEmployeesResponse.body.employees[0].employeeID
        );
    ...
    router.get("/pay-run-calendars", async (req: Request, res: Response) => {
      try {
        const response = await xero.payrollUKApi.getPayRunCalendars(
          req.session.activeTenant.tenantId
        );
    ...
    router.get("/salary-wages", async (req: Request, res: Response) => {
      try {
        ...
        const response = await xero.payrollUKApi.getEmployeeSalaryAndWages(
          req.session.activeTenant.tenantId,
          getEmployeesResponse.body.employees[0].employeeID
        );
    ...
    router.get("/pay-runs", async (req: Request, res: Response) => {
      try {
        const response = await xero.payrollUKApi.getPayRuns(
          req.session.activeTenant.tenantId
        );
    ...
    router.get("/payslips", async (req: Request, res: Response) => {
      try {
        const getPayRunsResponse = await xero.payrollUKApi.getPayRuns(
          req.session.activeTenant.tenantId
        );
        const response = await xero.payrollUKApi.getPaySlips(
          req.session.activeTenant.tenantId,
          getPayRunsResponse.body.payRuns[0].payRunID
        );
    ...
    router.get("/settings", async (req: Request, res: Response) => {
      try {
        const response = await xero.payrollUKApi.getSettings(
          req.session.activeTenant.tenantId
        );
        res.render("settings", {
          consentUrl: await xero.buildConsentUrl(),
    ...
    router.get("/tracking-categories", async (req: Request, res: Response) => {
      try {
        const response = await xero.payrollUKApi.getTrackingCategories(
          req.session.activeTenant.tenantId
        );
    ...
    router.get(
      ...
      async (req: Request, res: Response) => {
        try {
          const getDeductionsResponse = await xero.payrollNZApi.getDeductions(
            req.session.activeTenant.tenantId
          );
          // createDeduction
          // getDeduction
    ...
    router.get(
      ...
      async (req: Request, res: Response) => {
        try {
          const getEarningsRatesResponse =
            await xero.payrollNZApi.getEarningsRates(
              req.session.activeTenant.tenantId
            );
          // createEarningsRate
          // getEarningsRate
    ...
    router.get("/payroll-nz-employees", async (req: Request, res: Response) => {
      try {
        const getEmployeesResponse = await xero.payrollNZApi.getEmployees(
          req.session.activeTenant.tenantId
        );
        // createEmployee
        ...
        const createEmployeeResponse = await xero.payrollNZApi.createEmployee(
          req.session.activeTenant.tenantId,
          employee
        );
        employee.email = `tester${Helper.getRandomNumber(1000)}@gmail.com`;
        const updateEmployeeResponse = await xero.payrollNZApi.updateEmployee(
          req.session.activeTenant.tenantId,
          createEmployeeResponse.body.employee.employeeID,
          employee
        );
        const getEmployeeResponse = await xero.payrollNZApi.getEmployee(
          req.session.activeTenant.tenantId,
          updateEmployeeResponse.body.employee.employeeID
        );
    ...
    router.get(
      ...
      async (req: Request, res: Response) => {
        try {
          ...
          const getEmployeeLeavesResponse =
            await xero.payrollNZApi.getEmployeeLeaves(
              req.session.activeTenant.tenantId,
              getEmployeesResponse.body.employees[0].employeeID
            );
          // createEmployeeLeave
    ...
    router.get(
      ...
      async (req: Request, res: Response) => {
        try {
          ...
          const getEmployeeLeaveBalancesResponse =
            await xero.payrollNZApi.getEmployeeLeaveBalances(
              req.session.activeTenant.tenantId,
              getEmployeesResponse.body.employees[0].employeeID
            );
    ...
    router.get(
      "/payroll-nz-employees-leave-periods",
      async (req: Request, res: Response) => {
        try {
          // need an employee ID
          const getEmployeesResponse = await xero.payrollNZApi.getEmployees(
          ...
          const getEmployeeLeavePeriodsResponse =
            await xero.payrollNZApi.getEmployeeLeavePeriods(
              req.session.activeTenant.tenantId,
              getEmployeesResponse.body.employees[0].employeeID,
              "2019-01-01",
              "2020-12-31"
    ...
    router.get(
      "/payroll-nz-employees-leave-setup",
      async (req: Request, res: Response) => {
        try {
          // need an employee ID
          // const getEmployeesResponse = await xero.payrollNZApi.getEmployees(req.session.activeTenant.tenantId);
    ...
    router.get(
      "/payroll-nz-employees-leave-types",
      async (req: Request, res: Response) => {
        try {
          // need an employee ID
          const getEmployeesResponse = await xero.payrollNZApi.getEmployees(
          ...
          const getEmployeeLeaveTypesResponse =
            await xero.payrollNZApi.getEmployeeLeaveTypes(
              req.session.activeTenant.tenantId,
              getEmployeesResponse.body.employees[0].employeeID
            );
    ...
    router.get(
      "/payroll-nz-employees-opening-balances",
      async (req: Request, res: Response) => {
        try {
          // need an employee ID
          const getEmployeesResponse = await xero.payrollNZApi.getEmployees(
          ...
          const getEmployeeOpeningBalancesResponse =
            await xero.payrollNZApi.getEmployeeOpeningBalances(
              req.session.activeTenant.tenantId,
              getEmployeesResponse.body.employees[0].employeeID
            );
          ...
          res.render("payroll-nz-employees-opening-balances", {
            ...
            authenticated: this.authenticationData(req, res),
            openingBalances:
              getEmployeeOpeningBalancesResponse.body.openingBalances,
          });
        } catch (e) {
          res.status(res.statusCode);
          res.render("shared/error", {
    ...
    router.get(
      ...
      async (req: Request, res: Response) => {
        try {
          ...
          const getEmployeePaymentMethodResponse =
            await xero.payrollNZApi.getEmployeePaymentMethod(
              req.session.activeTenant.tenantId,
              getEmployeesResponse.body.employees[0].employeeID
            );
    ...
    router.get(
      "/payroll-nz-employees-pay-templates",
      async (req: Request, res: Response) => {
        try {
          // need an employee ID
          const getEmployeesResponse = await xero.payrollNZApi.getEmployees(
          ...
          const getEmployeePayTemplatesResponse =
            await xero.payrollNZApi.getEmployeePayTemplates(
              req.session.activeTenant.tenantId,
              getEmployeesResponse.body.employees[0].employeeID
            );
    ...
    router.get(
      ...
      async (req: Request, res: Response) => {
        try {
          ...
          const getEmployeeSalaryAndWagesResponse =
            await xero.payrollNZApi.getEmployeeSalaryAndWages(
              req.session.activeTenant.tenantId,
              getEmployeesResponse.body.employees[0].employeeID
            );
          ...
          res.render("payroll-nz-salary-wages", {
            ...
            authenticated: this.authenticationData(req, res),
            salaryAndWages:
              getEmployeeSalaryAndWagesResponse.body.salaryAndWages,
          });
        } catch (e) {
          res.status(res.statusCode);
          res.render("shared/error", {
    ...
    router.get(
      ...
      async (req: Request, res: Response) => {
        try {
          ...
          const getEmployeeTaxResponse = await xero.payrollNZApi.getEmployeeTax(
            req.session.activeTenant.tenantId,
            getEmployeesResponse.body.employees[0].employeeID
          );
          // updateEmployeeTax
    ...
    router.get(
      ...
      async (req: Request, res: Response) => {
        try {
          const getLeaveTypesResponse = await xero.payrollNZApi.getLeaveTypes(
            req.session.activeTenant.tenantId
          );
          // createLeaveType
          // getLeaveType
    ...
    router.get(
      ...
      async (req: Request, res: Response) => {
        try {
          const getPayRunCalendarsResponse =
            await xero.payrollNZApi.getPayRunCalendars(
              req.session.activeTenant.tenantId
            );
    ...
    router.get("/payroll-nz-pay-runs", async (req: Request, res: Response) => {
      try {
        const getPayRunsResponse = await xero.payrollNZApi.getPayRuns(
          req.session.activeTenant.tenantId
        );
        // createPayRun
        // getPayRun
    ...
    router.get("/payroll-nz-pay-slips", async (req: Request, res: Response) => {
      try {
        ...
        const getPaySlipsResponse = await xero.payrollNZApi.getPaySlips(
          req.session.activeTenant.tenantId,
          getPayRunsResponse.body.payRuns[0].payRunID
        );
        // getPaySlip
    ...
    router.get(
      ...
      async (req: Request, res: Response) => {
        try {
          const getReimbursementsResponse =
            await xero.payrollNZApi.getReimbursements(
              req.session.activeTenant.tenantId
            );
          // createReimbursement
          // getReimbursement
    ...
    router.get("/payroll-nz-settings", async (req: Request, res: Response) => {
      try {
        const getSettingsResponse = await xero.payrollNZApi.getSettings(
          req.session.activeTenant.tenantId
        );
    ...
    router.get(
      "/payroll-nz-tracking-categories",
      async (req: Request, res: Response) => {
        try {
          const getTrackingCategoriesResponse =
            await xero.payrollNZApi.getTrackingCategories(
              req.session.activeTenant.tenantId
            );
    ...
    router.get(
      ...
      async (req: Request, res: Response) => {
        try {
          const getTimesheetsResponse = await xero.payrollNZApi.getTimesheets(
            req.session.activeTenant.tenantId
          );
          // createTimesheet
          // deleteTimesheet
    ...
    router.get("/folders", async (req: Request, res: Response) => {
      try {
        const getFolders = await xero.filesApi.getFolders(
          req.session.activeTenant.tenantId
        );
        res.render("folders", {
          consentUrl: await xero.buildConsentUrl(),
    ...
    router.get("/files", async (req: Request, res: Response) => {
      try {
        const getFiles = await xero.filesApi.getFiles(
          req.session.activeTenant.tenantId
        );
        const getFolders = await xero.filesApi.getFolders(
          req.session.activeTenant.tenantId
        ...
        const uploadFile = await xero.filesApi.uploadFile(
          req.session.activeTenant.tenantId,
          readStream,
          filename,
          contentType
    ...
    router.get("/files-associations", async (req: Request, res: Response) => {
      try {
        ...
        const getInvoices = await xero.accountingApi.getInvoices(
          req.session.activeTenant.tenantId
        );
        ...
        const createAssociation = await xero.filesApi.createFileAssociation(
          req.session.activeTenant.tenantId,
          fileId,
          association
        );
    ...
    router.get("/accounting-activity", async (req: Request, res: Response) => {
      try {
        const getAccountingActivityAccountUsage =
          await xero.financeApi.getAccountingActivityAccountUsage(
            req.session.activeTenant.tenantId
          );
        const getAccountingActivityLockHistory =
          await xero.financeApi.getAccountingActivityLockHistory(
            req.session.activeTenant.tenantId
          );
        const getAccountingActivityReportHistory =
          await xero.financeApi.getAccountingActivityReportHistory(
            req.session.activeTenant.tenantId
          );
        const getAccountingActivityUserActivities =
          await xero.financeApi.getAccountingActivityUserActivities(
            req.session.activeTenant.tenantId
          );
        ...
        res.render("accounting-activity", {
          ...
          getAccountingActivityAccountUsage:
            getAccountingActivityAccountUsage.body,
          getAccountingActivityLockHistory:
            getAccountingActivityLockHistory.body,
          getAccountingActivityReportHistory:
            getAccountingActivityReportHistory.body,
          getAccountingActivityUserActivities:
            getAccountingActivityUserActivities.body,
        });
      } catch (e) {
        res.status(res.statusCode);
        res.render("shared/error", {
    ...
    router.get("/cash-validation", async (req: Request, res: Response) => {
      try {
        const getCashValidation = await xero.financeApi.getCashValidation(
          req.session.activeTenant.tenantId
        );
    ...
    router.get("/financial-statement", async (req: Request, res: Response) => {
      try {
        const getFinancialStatementBalanceSheet =
          await xero.financeApi.getFinancialStatementBalanceSheet(
            req.session.activeTenant.tenantId
          );
        const getFinancialStatementCashflow =
          await xero.financeApi.getFinancialStatementCashflow(
            req.session.activeTenant.tenantId
          );
        const getFinancialStatementProfitAndLoss =
          await xero.financeApi.getFinancialStatementProfitAndLoss(
            req.session.activeTenant.tenantId
          );
        const getFinancialStatementTrialBalance =
          await xero.financeApi.getFinancialStatementTrialBalance(
            req.session.activeTenant.tenantId
          );
        const getFinancialStatementContactsExpense =
          await xero.financeApi.getFinancialStatementContactsExpense(
            req.session.activeTenant.tenantId
          );
        const getFinancialStatementContactsRevenue =
          await xero.financeApi.getFinancialStatementContactsRevenue(
            req.session.activeTenant.tenantId
          );
    ...
    router.get("/bank-statements-plus", async (req: Request, res: Response) => {
      try {
        ...
        const getBankStatementsPlus =
          await xero.financeApi.getBankStatementAccounting(
            req.session.activeTenant.tenantId,
            accountId,
            fromDate,
            toDate
